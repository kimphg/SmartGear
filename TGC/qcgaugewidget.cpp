    ...
    QWidget(parent)
{
    ...
    initFigure();
    mSpeedNeedle->setCurrentValue(50);
}
...
void QcGaugeWidget::initFigure()
{
    double minAngle = -180;
    double maxAngle= 180;
    this->addBackground(99)->addColor(0.5,Qt::white);
    QcBackgroundItem *bkg1 = this->addBackground(97);
    bkg1->clearrColors();
    bkg1->addColor(0.1,QColor(179,188,182));
    bkg1->addColor(0.5,QColor(69,78,72));

    QcBackgroundItem *bkg2 = this->addBackground(88);
    bkg2->clearrColors();
    bkg2->addColor(0.1,QColor(69,78,72));
    bkg2->addColor(0.5,QColor(90,160,140));
    bkg2->addColor(1.0,QColor(69,78,72));

    this->addArc(55)->setColor(Qt::white);
    degreeItem = this->addDegrees(65);
    degreeItem->setValueRange(minAngle,maxAngle);
    degreeItem->setStep(30);
...
//    labitem->setColor(Qt::white);
    QcLabelItem *lab = this->addLabel(30);
    lab->setText("0");
    ...
    mSpeedNeedle->setColor(QColor(0,0,0));
    mSpeedNeedle->setValueRange(minAngle,maxAngle);
    this->addBackground(7);
//    this->addGlass(88);

}
void QcGaugeWidget::setValue(double value)
{


    mSpeedNeedle->setCurrentValue(value);
    degreeItem->setValueRange( -180+value,value+180);
     vlitem->setValueRange(-180+value,value+180);
}
...
void QcDegreesItem::draw(QPainter *painter)
{
    ...
    float r = getRadius(tmpRect);
    for(float val = ((int)mMinValue)/10*10;val<=((int)mMaxValue)/10*10;val+=mStep){
...
void QcNeedleItem::draw(QPainter *painter)
{
    ...
    case QcNeedleItem::WeaponNeedle:
        createGunNeedle(getRadius(tmpRect));
        break;
...
void QcNeedleItem::createGunNeedle(float r)
{
