...
bool KCF::Init(cv::Mat image, cv::Rect rect_init) {
    trackerMode = 1;
    timeline.clear();
    for(int i = 0;i<TRACK_MEM_SIZE;i++)
    {
        timeline.push_back(i);
        trackmax.push_back(0);
    }
...
cv::Rect KCF::getsearchingRect()
{
    cv::Point lefttop(min(frameW - 2, max(-window_sz_.width+1, pos_.x - cvFloor(float(window_sz_.width) / 2.0) + 1)),
                      min(frameH- 2, max(-window_sz_.height+1, pos_.y - cvFloor(float(window_sz_.height) / 2.0) + 1)));
    cv::Point rightbottom(lefttop.x + window_sz_.width, lefttop.y + window_sz_.height);

    cv::Rect border(max(lefttop.x, 0), max(lefttop.y, 0),
                    (rightbottom.x - lefttop.x), (rightbottom.y - lefttop.y));
    return border;
}
cv::Rect KCF::Update(cv::Mat image) {
    if(trackerMode==0)return result_rect_;
    frameH = image.rows;
    frameW = image.cols;
    cv::Point maxLoc;
    double maxVal;
    ...
    if(trackerMode==1)
    {
        if(!detectTrackLost(maxVal))
        {
            ...
            double learnFactor = maxVal/1.2/trackmean;
            if(learnFactor>1)learnFactor=1;
            Learn(patch, learning_rate*learnFactor);
            return result_rect_;
        }
        else
        {
    ...
    {
        trackmax.push_back(maxVal);
        while(trackmax.size()>TRACK_MEM_SIZE)trackmax.pop_front();
        if(maxVal>trackmean*0.7)
        {
            trackerMode=1;
        }
    }
...
bool KCF::detectTrackLost(double matchingScore)
{
    trackmax.push_back(matchingScore);
    trackmean+=(matchingScore-trackmean)*0.03;
    //    printf("meanval: %f \n",trackmean);
    while(trackmax.size()>TRACK_MEM_SIZE)trackmax.pop_front();

    if(matchingScore<trackmean/trackLostSens)
    {
        return true;
    }
    return false;
...
void KCF::setLearning_rate(float value)
{
    learning_rate = value;
}

void KCF::setInterp_factor(float interp_factor)
{
    learning_rate = interp_factor;
}
